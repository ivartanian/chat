      ...
      if [ "$INVOKER" == "yarn-daemon.sh" ]; then
        export YARN_ROOT_LOGGER=${YARN_ROOT_LOGGER:-ERROR,EWMA,RFA}
      fi
      ...
      YARN_OPTS="$YARN_OPTS -Dhadoop.root.logger=${YARN_ROOT_LOGGER:-ERROR,console}"
      YARN_OPTS="$YARN_OPTS -Dyarn.root.logger=${YARN_ROOT_LOGGER:-ERROR,console}"
